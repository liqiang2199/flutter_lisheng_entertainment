...
class _TrendHanoiOneLotteryController extends BaseController<TrendHanoiOneLotteryController> with AutomaticKeepAliveClientMixin{

  List<TrendCoordinateBeen> trendPointList = new List();
  /// 期数 多少选择 30 50 100
  List<bool> numberOfPeriodsNum = [true, false, false];
  List<String> trendTitleList = ["期数", "1", "2", "3", "4", "5", "6", "7", "8", "9"];
  ...
  Widget build(BuildContext context) {
    ...
    screenW = (ScreenUtil.getScreenW(context) - numberOfPeriodsLength) / 9;
...
//        new Expanded(child: _cpList(),),
  ...
  Widget _trendContentView(int color, int index, String title) {
    if (index != -1) {
      var randomNext = (Random().nextInt(8) + 2);
